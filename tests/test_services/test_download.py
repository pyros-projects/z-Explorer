...
class TestGetModelsToDownload:
    ...
    def test_returns_empty_for_local_mode(self):
        ...
        from z_explorer.model_config import LLMMode, LoadingMode
    ...
    def test_returns_sdnq_model(self):
        ...
        with patch("z_explorer.model_config.get_image_model_config") as mock_img:
            with patch("z_explorer.model_config.get_llm_config") as mock_llm:
                mock_img.return_value = MagicMock(
                    mode=LoadingMode.SDNQ, sdnq_model="test/sdnq-model"
                )
                mock_llm.return_value = MagicMock(mode=LLMMode.HF_LOCAL)
        ...
        assert len(models) == 1
        # Model name is now the repo_id itself for model-agnostic display
        assert models[0][0] == "test/sdnq-model"
    ...
    def test_returns_both_models(self):
        ...
        with patch("z_explorer.model_config.get_image_model_config") as mock_img:
            with patch("z_explorer.model_config.get_llm_config") as mock_llm:
                mock_img.return_value = MagicMock(
                    mode=LoadingMode.HF_DOWNLOAD, hf_repo="test/image-model"
                )
                mock_llm.return_value = MagicMock(
                    mode=LLMMode.HF_DOWNLOAD, hf_repo="test/llm-model"
                )
...
class TestDownloadAllModels:
    ...
    def test_returns_true_when_no_models_needed(self):
        ...
        with patch(
            "z_explorer.services.download.get_models_to_download", return_value=[]
        ):
    ...
    def test_downloads_all_models(self):
        ...
        with patch(
            "z_explorer.services.download.get_models_to_download",
            return_value=[("Model1", "repo1"), ("Model2", "repo2")],
        ):
            with patch(
                "z_explorer.services.download.download_model_with_progress",
                return_value=True,
            ) as mock_download:
                result = download_all_models()
...
class TestCheckModelsDownloaded:
    ...
    def test_returns_dict_of_model_status(self):
        ...
        # Create mock cache info with the repo we're looking for
        mock_repo = MagicMock()
        mock_repo.repo_id = "repo1"
    ...
    def test_returns_false_for_uncached(self):
        ...
        # Create mock cache info with no matching repos
        mock_cache_info = MagicMock()
        mock_cache_info.repos = []  # Empty cache
