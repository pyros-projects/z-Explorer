...
def temp_library(tmp_path):
    ...
    (lib_path / "art_style.md").write_text(
        "# Art styles\nwatercolor\noil painting\nsketch\ndigital art"
    )
    ...
    sub_dir.mkdir()
    (sub_dir / "creature.md").write_text(
        "# Fantasy creatures\ndragon\nunicorn\nphoenix"
    )
...
def mock_image_generator():
    ...
    with patch("z_explorer.image_generator._pipeline") as mock_pipeline:
        ...
        with patch(
            "z_explorer.image_generator.generate_image", side_effect=mock_generate
        ):
            yield mock_pipeline
...
def mock_llm_provider():
    ...
    with patch("z_explorer.llm_provider._model") as mock_model:
        ...
        with patch("z_explorer.llm_provider.enhance_prompt", side_effect=mock_enhance):
            with patch(
                "z_explorer.llm_provider.generate_prompt_variable_values",
                side_effect=mock_generate_vars,
            ):
