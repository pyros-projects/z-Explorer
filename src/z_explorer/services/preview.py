...
try:
    ...
    logger.warning(
        "`term-image` library not found. Terminal previews will be disabled."
    )
    logger.warning("Install with: pip install term-image")
    TERM_IMAGE_SUPPORTED = False
except Exception as e:
    logger.warning(
        f"Could not initialize `term-image` ({e}). Terminal previews disabled."
    )
    TERM_IMAGE_SUPPORTED = False
    ...
    def __init__(self, term_image_obj: "BaseImage"):
        if TERM_IMAGE_SUPPORTED:
            if not isinstance(term_image_obj, BaseImage):
                raise TypeError(
                    f"Internal Error: Expected a term_image BaseImage object, got {type(term_image_obj)}"
                )
        elif term_image_obj is not None:
            raise TypeError(
                "Internal Error: Received an object for TermImageRenderable when term-image is not supported."
            )
...
def does_terminal_support_images() -> bool:
    ...
    if not TERM_IMAGE_SUPPORTED:
        ...
        try:
            ...
            logger.warning(
                f"Terminal image support check failed ({e}). Disabling previews."
            )
            support_result = False
    ...
    logger.debug(
        f"Terminal support check complete. Result: {_terminal_supports_images}"
    )
    return _terminal_supports_images
...
async def get_preview_renderable(
    image_bytes: bytes,
) -> Optional[Union[TermImageRenderable, Text]]:  # Return type changed
    """
...
def display_final_image_os(image_path: str):
    ...
    try:
        ...
        logger.error(
            f"Command not found to open image viewer ('open' or 'xdg-open'). Please open manually: {image_path}"
        )
    except Exception as e:
