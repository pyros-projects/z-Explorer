...
def get_changelog_path() -> Path:
    ...
    possible_paths = [
        ...
        / "CHANGELOG.md",  # From src/z_explorer/
...
class PromptCompleter(Completer):
    ...
    def get_completions(self, document, complete_event):
        ...
        if "__" in text_before_cursor:
            ...
            if (
                cursor_position > last_var_start
                and text_before_cursor.count("__", last_var_start) == 1
            ):
                ...
                if ":" not in current_var:  # Not typing an index
                    for var in self.vars:
                        if var.startswith(current_var):
                            ...
                            yield Completion(
                                completion_text, start_position=0, display=var
                            )
...
def parse_batch_params(user_input: str) -> tuple[str, dict]:
    ...
    if not any(
        p.strip().startswith(("x", "h", "w")) and any(c.isdigit() for c in p)
...
def print_banner():
    ...
    console.print(
        f"[dim]v{__version__} • Fully offline image generation with Z-Image-Turbo + Qwen3-4B[/dim]"
    )
    console.line()
    ...
    for match in matches:
        ...
        if var_name in prompt_vars:
            ...
            if values:
                ...
                console.print(
                    f"[green]✓ Generated {len(values)} values, saved to {file_path}[/green]"
                )
                ...
                console.print(
                    f"[yellow]⚠ Could not generate values for {var_name}[/yellow]"
                )
...
def enhance_prompt_local(prompt: str) -> str:
    ...
    console.print("[green]✓ Enhanced prompt ready[/green]")
    return enhanced


def generate_image_local(
    prompt: str, seed: int = None, width: int = 1024, height: int = 1024
) -> tuple:
...
def show_help():
    ...
    console.print("\n[dim]Tips:[/dim]")
    console.print(
        "[dim]• Use __variable__ syntax for random substitution (e.g., __animal__)[/dim]"
    )
    console.print("[dim]• Missing variables are auto-generated by the local LLM[/dim]")
    console.print(
        "[dim]• Add '>' to enhance prompt (e.g., 'a cat > make it magical')[/dim]"
    )
    console.print(
        "[dim]• Add batch params with ':' (e.g., 'prompt : x10,h832,w1216')[/dim]"
    )
    ...
    while True:
        try:
            ...
            if user_input.startswith("/"):
                ...
                if cmd in ["/quit", "/q", "/exit"]:
                    ...
                    if args:
                        ...
                        console.print(
                            Panel(
                                enhanced, title="Enhanced Prompt", border_style="green"
                            )
                        )
                    else:
                    ...
                    if args:
                        try:
                            ...
                            console.print(
                                f"[green]Size set to: {current_size[0]}x{current_size[1]}[/green]"
                            )
                        except:
                            console.print(
                                "[red]Invalid size format. Use: /size 1024x1024[/red]"
                            )
                    else:
                        console.print(
                            f"[dim]Current size: {current_size[0]}x{current_size[1]}[/dim]"
                        )
                    ...
                    console.print(
                        "[dim]https://github.com/pyros-projects/z-Explorer[/dim]"
                    )
                    ...
                    if changelog_path:
                        ...
                        console.print(
                            "[dim]https://github.com/pyros-projects/z-Explorer/blob/main/CHANGELOG.md[/dim]"
                        )
            ...
            if count > 1 or img_height != 1024 or img_width != 1024:
                console.print(
                    f"[dim]Batch: {count} image(s) @ {img_width}x{img_height}[/dim]"
                )
            ...
            for i in range(count):
                ...
                if ">" in prompt_input:
                    ...
                    if count == 1:
                        console.print(
                            f"[cyan]🤖 Enhancing with instruction: {instruction}[/cyan]"
                        )
                    ...
                    if count == 1:
                        console.print(
                            Panel(prompt, title="Enhanced Prompt", border_style="cyan")
                        )
                else:
            ...
            for i, prompt in enumerate(generated_prompts):
                ...
                console.print(
                    Panel(prompt, title=f"Prompt {i + 1}", border_style="green")
                )

                image, path = generate_image_local(
                    prompt, seed=current_seed, width=img_width, height=img_height
                )
    ...
    for gui_dir in gui_locations:
        ...
        if package_json.exists() and not dist_dir.exists():
            ...
            try:
                ...
                console.print(
                    "[red]npm not found. Please install Node.js to build the GUI.[/red]"
                )
                return False
            except Exception as e:
...
def start_web_mode(port: int, host: str = "127.0.0.1"):
    ...
    console.print(
        f"[cyan]🚀 Starting Z-Explorer on http://{display_host}:{port}[/cyan]"
    )

    if host == "0.0.0.0":
        console.print(
            "[yellow]⚠ Server bound to all interfaces (0.0.0.0) - accessible from network[/yellow]"
        )
    ...
    if not _check_dependencies():
        console.print(
            Panel(
                ...
                "[cyan]uv sync[/cyan]\n\n"
                "Required packages:\n"
                "[dim]torch, transformers, diffusers[/dim]",
  ...
  z-explorer --host 0.0.0.0 Bind to all interfaces (network accessible)
    ...
    if args.quick_setup:
        ...
        if is_configured():
            ...
            console.print(
                "[green]✓ Quick Start configuration saved (SDNQ + quantized LLM, ~12GB VRAM)[/green]"
            )
    ...
    if not is_configured():
        ...
        if run_now:
            ...
            console.print(
                "[dim]Using Quick Start defaults (SDNQ + quantized LLM, ~12GB VRAM)[/dim]\n"
            )
