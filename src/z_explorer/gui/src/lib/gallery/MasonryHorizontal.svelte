<script lang="ts">
  /**
   * MasonryHorizontal Layout
   * Horizontal lanes with variable widths
   * Best for landscape-heavy galleries
   */
  ...
  function getWidth(img: ImageData): number {
    const aspect = imageAspects.get(img.url) || 1;
    return Math.round(thumbnailHeight * aspect);
  }
</script>

<div class="masonry-horizontal" style="--thumb-height: {thumbnailHeight}px">
  {#each images as img, i}
    <button
      class="image-card"
      on:click={() => onSelect(img)}
      style="animation-delay: {i * 50}ms; width: {getWidth(img)}px"
    >
      <img
        src={img.url}
