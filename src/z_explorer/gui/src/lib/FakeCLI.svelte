<script lang="ts">
      ...
      if (configRes.ok) {
        ...
        history = history.map(item => {
          if (item.type === 'info' && item.text.includes('Image:')) {
            return { ...item, text: `🎨 Image: ${config.image_model} (${config.image_mode})  •  🤖 LLM: ${config.llm_model} (${config.llm_mode})` };
          }
          return item;
        });
      }
  ...
  async function loadVariables() {
    console.log('📚 [FakeCLI] Loading variables...');
    if (listVariables) {
      try {
        const result = await listVariables();
        console.log('📚 [FakeCLI] Variables result:', result);
        if (result?.success && result.variables) {
          promptVars = result.variables.map((v: any) => v.id);
          console.log('✅ [FakeCLI] Loaded variables for autocomplete:', promptVars);
        } else {
          console.warn('⚠️ [FakeCLI] No variables returned');
        }
      } catch (e) {
        console.error('❌ [FakeCLI] Could not load variables:', e);
      }
    } else {
      console.warn('⚠️ [FakeCLI] listVariables prop not provided');
    }
  }
  ...
  onMount(async () => {
    ...
    history = [
      { type: 'info', text: `🎨 Image: ${modelConfig.image_model} (${modelConfig.image_mode})  •  🤖 LLM: ${modelConfig.llm_model} (${modelConfig.llm_mode})` },
      { type: 'success', text: '✓ All dependencies available' },
    ];
  ...
  const tipsList = [
    '• Use __variable__ syntax for random substitution (e.g., __animal__)',
    '• Missing variables are auto-generated by the local LLM',
    '• Add \'>\' to enhance prompt (e.g., \'a cat > make it magical\')',
    '• Add batch params with \':\' (e.g., \'prompt : x10,h832,w1216\')',
  ];
  ...
  function updateSuggestions() {
    ...
    if (textBeforeCursor.includes('/')) {
      ...
      console.log(`🔍 [Autocomplete] Command suggestions for "${partial}":`, suggestions);
      return;
    }
    ...
    if (textBeforeCursor.includes('__')) {
      ...
      console.log(`🔍 [Autocomplete] Looking for variables matching "${afterVar}"`);
      console.log(`🔍 [Autocomplete] Available promptVars:`, promptVars);
      if ((afterVar.match(/__/g) || []).length === 1) {
        suggestions = promptVars.filter(v => v.startsWith(afterVar) && v !== afterVar);
        showSuggestions = suggestions.length > 0;
        console.log(`🔍 [Autocomplete] Variable suggestions:`, suggestions);
        return;
      }
    }
  ...
  function submit() {
    ...
    if (params.count > 1 || params.width !== 1024 || params.height !== 1024) {
      history = [...history, {
        type: 'info',
        text: `📐 Batch: ${params.count} image(s) @ ${params.width}×${params.height}`
      }];
    }
    ...
    switch (command) {
      case '/help':
        history = [...history,
          ...
          { type: 'divider' },
          { type: 'cyan', text: '💡 Tips:' },
          { type: 'dim', text: '• Use __variable__ syntax for random substitution (e.g., __animal__)' },
          { type: 'dim', text: '• Missing variables are auto-generated by the local LLM' },
          { type: 'dim', text: '• Add \'>\' to enhance prompt (e.g., \'a cat > make it magical\')' },
          { type: 'dim', text: '• Add batch params with \':\' (e.g., \'prompt : x10,h832,w1216\')' },
          { type: 'dim', text: '  x<N>=count, h<N>=height, w<N>=width (all optional)' },
        ];
        break;
      case '/vars':
        if (listVariables) {
          history = [...history, { type: 'cyan', text: '📋 Loading variables...' }];
          scrollToBottom();
          ...
          listVariables().then((result: any) => {
            if (result?.success && result.variables) {
              ...
              history = [...history,
                { type: 'header', text: `📋 ${result.count} Variables Available` },
                { type: 'varsTable', rows: varRows },
              ];
              // Update autocomplete
              promptVars = result.variables.map((v: any) => v.id);
            } else {
              history = [...history, { type: 'error', text: '❌ Failed to load variables' }];
            }
            scrollToBottom();
          });
        } else {
          history = [...history,
            { type: 'header', text: '📋 Available Variables' },
            { type: 'divider' },
      ...
      case '/enhance':
        if (args.length > 0) {
          history = [...history,
            { type: 'cyan', text: '🤖 Enhancing prompt with local LLM...' },
            { type: 'dim', text: '(Enhancement will be applied during generation)' },
          ];
        } else {
          history = [...history,
            { type: 'error', text: '❌ Usage: /enhance <prompt>' },
          ];
        }
        break;
      case '/seed':
        if (args.length > 0) {
          ...
          if (!isNaN(seedVal) && seedVal >= 0) {
            currentSeed = seedVal;
            history = [...history,
              { type: 'success', text: `✓ Seed set to: ${seedVal}` },
              { type: 'dim', text: '(Will be used for next generation, then reset)' },
            ];
          } else {
            history = [...history,
              { type: 'error', text: '❌ Invalid seed (must be a positive number)' },
            ];
          }
        } else {
          currentSeed = null;
          history = [...history,
            { type: 'cyan', text: '🎲 Seed reset to random' },
          ];
        }
        break;
      case '/size':
        if (args.length > 0) {
          ...
          if (sizeMatch) {
            history = [...history,
              { type: 'success', text: `✓ Size set to: ${sizeMatch[1]}×${sizeMatch[2]}` },
            ];
          } else {
            history = [...history,
              { type: 'error', text: '❌ Invalid size format. Use: /size 1024x1024' },
            ];
          }
        } else {
          history = [...history,
            { type: 'cyan', text: '📐 Current size: 1024×1024' },
          ];
        }
        break;
      case '/gpu':
        if (getGpuInfo) {
          history = [...history, { type: 'cyan', text: '🖥️ Fetching GPU info...' }];
          scrollToBottom();

          getGpuInfo().then((info: any) => {
            if (info?.available) {
              history = [...history,
                { type: 'header', text: '🖥️ GPU Memory Status' },
              ...
              history = [...history,
                { type: 'error', text: `❌ ${info?.error || 'GPU not available'}` },
              ];
            }
            scrollToBottom();
          });
      ...
      case '/unload':
        if (unloadModels) {
          history = [...history, { type: 'cyan', text: '🔄 Unloading models...' }];
          scrollToBottom();
          ...
          unloadModels().then((result: any) => {
            if (result?.success) {
              ...
              if (unloaded.length > 0) {
                history = [...history,
                  { type: 'success', text: '✓ Models unloaded successfully' },
                  { type: 'dim', text: `Unloaded: ${unloaded.join(', ')}` },
                  { type: 'dim', text: '(Models will reload on next generation)' },
                ];
              } else {
                history = [...history,
                  { type: 'info', text: 'ℹ️ No models were loaded' },
                ];
              }
            } else {
              history = [...history,
                { type: 'error', text: '❌ Failed to unload models' },
              ];
            }
            scrollToBottom();
          });
      ...
      case '/version':
        history = [...history,
          { type: 'header', text: `⚡ Z-Explorer v${VERSION}` },
      ...
      case '/changelog':
        history = [...history,
          ...
          { type: 'divider' },
          { type: 'success', text: '✨ Added:' },
          { type: 'dim', text: '• Native desktop app via Tauri with bundled uv binary' },
          { type: 'dim', text: '• Three modes: GUI (default), CLI (--cli), Server (--server)' },
          { type: 'dim', text: '• Progress streaming via Server-Sent Events (SSE)' },
          { type: 'dim', text: '• Masonry gallery layout for generated images' },
          { type: 'dim', text: '• Prompt saved alongside images as .txt files' },
          { type: 'dim', text: '• Variable autocomplete in CLI-style input' },
          { type: 'dim', text: '• /changelog and /version commands' },
          { type: 'divider' },
          { type: 'info', text: '🔄 Changed:' },
          { type: 'dim', text: '• Default mode is now GUI (previously CLI-only)' },
          { type: 'dim', text: '• Server port standardized to 8345' },
          { type: 'divider' },
      ...
      case '/settings':
        if (openSettings) {
          history = [...history,
            { type: 'cyan', text: '⚙️ Opening settings...' },
          ];
          openSettings();
        } else {
          history = [...history,
      ...
      case '/q':
        history = [...history,
          { type: 'cyan', text: '👋 Goodbye! (Close the window to exit)' },
        ];
        break;
      default:
        history = [...history,
  ...
  export function startGeneration() {
    console.log('🎬 [FakeCLI] startGeneration() called');
    currentProgress = 0;
    currentStage = 'starting';
    progressBarIndex = history.length;
    history = [...history, { type: 'progress-bar', percent: 0, stage: 'starting' }];
    console.log('📊 [FakeCLI] Progress bar added at index:', progressBarIndex);
    scrollToBottom();
  }
  ...
  export function updateProgress(percent: number, stage: string, message: string) {
    console.log(`📈 [FakeCLI] updateProgress(${percent}%, ${stage}, "${message}")`);
    ...
    if (progressBarIndex >= 0 && progressBarIndex < history.length) {
      ...
      console.log(`📊 [FakeCLI] Progress bar updated: ${percent}% - ${stage}`);
    } else {
      console.warn('⚠️ [FakeCLI] Progress bar index invalid:', progressBarIndex);
    }
    ...
    if (message && !silentStages.has(stage)) {
      ...
      if (stage === 'enhanced') {
        ...
        console.log(`💡 [FakeCLI] Skipping enhanced prompt display (will show in final)`);
      } else if (stage === 'final_prompt') {
        history = [...history, { type: 'prompt-highlight', variant: 'final', text: message }];
        console.log(`📜 [FakeCLI] Added final prompt: ${message.substring(0, 50)}...`);
      } else {
        history = [...history, { type: 'progress-msg', text: `${icon} ${message}` }];
        console.log(`💬 [FakeCLI] Added progress message: ${icon} ${message}`);
      }
    } else if (silentStages.has(stage)) {
      console.log(`🔇 [FakeCLI] Silent stage, only updating bar: ${stage}`);
    }
  ...
  export function completeGeneration(success: boolean) {
    console.log(`🏁 [FakeCLI] completeGeneration(success=${success})`);
    ...
    if (progressBarIndex >= 0 && progressBarIndex < history.length) {
      ...
      history = [...history];
      console.log('✅ [FakeCLI] Progress bar set to complete');
    }
  ...
  <div class="history" class:hidden={!historyVisible} bind:this={historyEl}>
    ...
    {#if $settings.cli.showTutorialOnStart || tutorialExpanded}
      <div class="tutorial-section">
        ...
        >
          <span class="toggle-icon">{tutorialExpanded ? '▼' : '▶'}</span>
    ...
    {#if $settings.cli.showTipsOnStart || tipsExpanded}
      <div class="tips-section">
        ...
        >
          <span class="toggle-icon">{tipsExpanded ? '▼' : '▶'}</span>
